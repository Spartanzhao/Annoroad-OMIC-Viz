...
def draw_insulation(ax,insu,chrs,start,end):
    df_insu=cut_insulation(insu,chrs,start,end)
    ax.tick_params(top='off',bottom='off',left='on',right='off')
    line=ax.plot(df_insu['mid'],df_insu['insu'], color='k', linewidth=0.8, label="insulation")
    ax.set_xlim(start,end)
    ax.set_xticks([])
...
def draw_AB(files,res,chrom,start,end,sample,ax):
    compartdf = pd.read_table(files,sep='\t',names=['sample','resolution','chrom','path'])
    compartdf = compartdf[compartdf['chrom']==chrom]
    compartdf = compartdf[compartdf['resolution']==res]
    compartdf = compartdf[compartdf['sample']==sample]
    compartdf = compartdf.reset_index(drop=True)
    compdf   = pd.read_table(compartdf['path'][0],header=0,index_col=False,sep='\t')
    if len(list(compdf.columns))==11:
        compdf.columns = ['chr','start','end','index','eigen1','eigen1evr','eigen2','eigen2evr','eigen3','eigen3evr','geneDensity']
    elif len(list(compdf.columns))==12:
        compdf.columns = ['chr','start','end','name','index','eigen1','eigen1evr','eigen2','eigen2evr','eigen3','eigen3evr','geneDensity']
    print (compdf.columns)
    compdf   = compdf.loc[:,['chr','start','end','eigen1']]
    df = compdf
    df=df[df['end']>=start]
    df=df[df['start']<=end]
...
def draw_diff_epigenetic(file1,file2,ax,chrs,start,end,color,MaxYlim,type):
    ...
    if MaxYlim == 'None':
        ...
        yminlim = 0
    ax.set_xlim([start,end])
    ax.set_ylim([-0.8*ymaxlim,0.8*ymaxlim])
    xts   = np.linspace(start,end,5)
    yts   = np.linspace(0.8*yminlim,0.8*ymaxlim,2)
    xtkls = ['{:,}'.format(int(i)) for i in xts]
    ytkls = ['{:,}'.format(int(j)) for j in yts]
...
def main():
    ...
    while j < len(file_list):
        ...
        if file_list[j][0]=='matrix':
            ...
            x+=dic[file_list[j][0]]
            ax = fig.add_axes([0.1,1-(x/l)-x/(4*l),0.8,(1/l)*0.65])
            draw_insulation(ax,file_list[j][2],chrs,start,end)
            j+=1
            ...
            draw_diff_epigenetic(file_list[y][3],file_list[z][3],ax,chrs,start,end,file_list[j][3],file_list[j][4],'none')
            j+=1
